...
function ChannelsList() {
  ...
  useEffect(() => {
    async function fetchChannels() {
      try {
        const response = currentUserId && await axios.get<channel[]>(`${process.env.NEXT_PUBLIC_APP_URI}:3000/channel/${currentUserId}/myAllChannels`);
  ...
  useEffect(() => {
    async function fetchChannels() {
      try {
        const response = currentUserId && await axios.get<channel[]>(`${process.env.NEXT_PUBLIC_APP_URI}:3000/channel/${currentUserId}/allChannels`);
  ...
  useEffect(() => {
    async function fetchPrivateChannel() {
      try {
        const response = privateInputId && await axios.get<channel>(`${process.env.NEXT_PUBLIC_APP_URI}:3000/channel/${privateInputId}`);
