  ...
  return (
    <div className='friend'>
      ...
      <div className="w-full flex flex-col gap-[50px] items-center mt-[50px]">
        ...
        <ul className="w-full flex flex-col gap-[20px] min-w-[300px] max-w-[400px]">
            {
              online_array.map((friend, index) => (
                <>
                <li key={index} className="flex justify-between items-center">
                  <div className="relative inline-block flex items-center gap-[10px]">
                  ...
                  <div className="friend_message">
                    <Image key={`${friend.name}-${friend.id}`} src={message_w} width={50} alt={friend.name} />
                  </div>
                </li>
                <hr />
                </>
