...
function Friendsbar({ userId, userSessionId }: Props) {
    ...
    useEffect(() => {
        const fetchFriends = async () => {
            try {
              const response = await axios.get(`http://127.0.0.1:3000/users/${userSessionId}/friend`);
                ...
                <li key={index} className="flex justify-between items-center">
                  <Link href={`${friend.id}`} >
                    <div className="relative flex items-center gap-[10px] w-[100px]">
                      ...
                      <Image src={friend?.Avatar} width={70} height={70} alt={friend?.name} className="rounded-full max-w-[70px] max-h-[70px] w-[70px] h-[70px] object-cover"/>
