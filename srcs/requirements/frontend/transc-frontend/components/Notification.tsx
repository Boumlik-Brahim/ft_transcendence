...
import { close_r, message_b, notification_b, sms } from "../public";
...
function Notification({ userId, userSession }: props) {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      try {
        const response = userSession && (await axios.get(`http://localhost:3000/chat/${userSession}`));
  ...
  /* ------------------------------- getPending ------------------------------- */
  const pendingMessages = usePendingMessage(messages, userSession);
  /* ------------------------------------ - ----------------------------------- */
          ...
          <li>
            <div className="relative flex items-center gap-[10px] w-[100px]">
              <Image
                ...
                className="rounded-full max-w-[50px] max-h-[50px] w-[40px] h-[40px] object-cover"
              />
              <p className="text-primary font-semibold">OSAMA</p>
            </div>
          </li>
        </ul>
      )}
